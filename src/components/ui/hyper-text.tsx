...
export function HyperText({
    ...
    useEffect(() => {
        ...
        const animate = (currentTime: number) => {
            ...
            const revealed = Math.floor(iterationCount.current);
            const set = characterSet.length ? characterSet : DEFAULT_CHARACTER_SET;

            setDisplayText((currentText) =>
                currentText.map((letter, index) =>
                    letter === ' ' ? letter : index <= revealed ? children[index] : set[getRandomInt(set.length)],
                ),
            );
        ...
        >
            <AnimatePresence>
                {displayText.map((letter, index) => (
                    <motion.span key={index.toString()} className={cn('font-mono', letter === ' ' ? 'w-3' : '')}>
                        {letter.toUpperCase()}
                    </motion.span>
                ))}
            </AnimatePresence>
